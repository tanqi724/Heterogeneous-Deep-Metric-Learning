function [F,D] = embedding_loss_incomplete(w,X,Y,LFinal,W,w_ind,XMiss)
...
%
    ...
    x = zeros(hidden_size,N,np); % hidden_size, # of locations, # of period
    W{w_ind} = w;
    for i = 1 : length(X)      
        for p = 1: size(X{i},3)
    ...
    for tt = 2 : T
        ...
        if nargout > 1
           % Compute gradient
           S = mlkrGrad2(X{w_ind}(:,:,tPeriod),y,yhat',Kij, w,XMiss{w_ind}, x(:,:,tPeriod)); % S: H*D
           D = vec( ((LFinal'*LFinal)+ (LFinal'*LFinal)') *S);           
           D_Final = D_Final + D;
        end
    end
...
function M = mlkrGrad2(X,Y,yhat,Kij,w,xmiss, fx)
    N = size(Y,2);
    ...
    for i = xmiss'
        for j = 1: N
            M = M + O(i,j)*( (fx(:,i) - fx(:,j)) .* fx_s(:,i)*X(:,xmiss==i)'  );
        end
    end
    
    for i = 1: N
        for j = xmiss'
            M = M + O(i,j)*(  - (fx(:,i) - fx(:,j)).* fx_s(:,j)*X(:,xmiss==j)' );
        end
    end
