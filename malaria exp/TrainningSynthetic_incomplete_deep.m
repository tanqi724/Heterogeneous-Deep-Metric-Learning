...
while OutIter < 5
    ...
    while iter < 6    
        ...
        %[LFinal,hist] = minimize(LFinal(:),'mmlkrLoss_global',pars.maxiter,x(:,:,1:end-1),residule,LFinal,lambda); % LFinal,lambda are not used anymore.        
        %LFinal = reshape(LFinal,Hidden_size,Hidden_size);
